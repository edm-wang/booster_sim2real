...
class Controller:
    ...
    def _init_low_state_values(self):
        ...
        self.base_linvel = np.zeros(3, dtype=np.float32)  # May not be available from SDK
    ...
    def _low_state_handler(self, low_state_msg: LowState):
        ...
        if time_now >= self.next_inference_time:
            ...
            # Note: base_linvel is not directly available from LowState
            # We'll pass None and let the policy use zero or estimate
            for i, motor in enumerate(low_state_msg.motor_state_serial):
