...
class StandaloneWebPlotter:
    ...
    def _update_buffers(self, obs, timestamp):
        ...
        # Extract components - handle both 47-dim (deploy_booster) and 85-dim (deploy_playground) formats
        if len(obs) >= 47:  # deploy_booster format
            # Gravity vector (3)
            self.gravity_data.append(obs[0:3])
            # Angular velocity (3)
            self.angular_vel_data.append(obs[3:6])
            # Commands (3) - vx, vy, vyaw
            self.commands_data.append(obs[6:9])
            # Gait phase (2) - cos, sin
            self.gait_phase_data.append(obs[9:11])
            
            # Joint positions (12) - only leg joints
            joint_pos = obs[11:23]
            self.joint_pos_data.append([np.mean(joint_pos), np.std(joint_pos)])
            
            # Joint velocities (12) - only leg joints
            joint_vel = obs[23:35]
            self.joint_vel_data.append([np.mean(joint_vel), np.std(joint_vel)])
            
            # Actions (12) - only leg actions
            actions = obs[35:47]
            self.actions_data.append([np.mean(actions), np.std(actions)])
            
            # For linear velocity, we don't have it in deploy_booster format, so use zeros
            self.linear_vel_data.append([0.0, 0.0, 0.0])
            
        elif len(obs) >= 85:  # deploy_playground format
    ...
    def _create_plots(self):
        ...
        try:
            ...
            safe_plot(axes[1, 1], times, list(self.gait_phase_data), ['Cos', 'Sin'], 
            ...
            safe_plot(axes[2, 1], times, list(self.actions_data), ['Mean', 'Std'], 
                     'Policy Actions (mean/std)', 'Action Value')
            
            # Summary plot
            if self.current_obs is not None and len(self.current_obs) > 0:
                try:
                    # Handle different observation sizes (47 for deploy_booster, 85 for deploy_playground)
                    obs_len = len(self.current_obs)
                    
                    if obs_len >= 47:  # deploy_booster format
                        summary_data = [
                            np.linalg.norm(self.current_obs[0:3]) if len(self.current_obs[0:3]) > 0 else 0,
                            np.linalg.norm(self.current_obs[3:6]) if len(self.current_obs[3:6]) > 0 else 0,
                            np.linalg.norm(self.current_obs[6:9]) if len(self.current_obs[6:9]) > 0 else 0,
                            np.linalg.norm(self.current_obs[9:11]) if len(self.current_obs[9:11]) > 0 else 0,
                            np.mean(np.abs(self.current_obs[11:23])) if len(self.current_obs[11:23]) > 0 else 0,
                            np.mean(np.abs(self.current_obs[23:35])) if len(self.current_obs[23:35]) > 0 else 0,
                            np.mean(np.abs(self.current_obs[35:47])) if len(self.current_obs[35:47]) > 0 else 0
                        ]
                        summary_labels = ['Gravity', 'AngVel', 'Commands', 'Gait', 'JointPos', 'JointVel', 'Actions']
                    ...
                    summary_data = [0 if (np.isnan(x) or np.isinf(x)) else x for x in summary_data]
