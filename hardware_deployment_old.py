...
class HardwareController:
    ...
    def _init_booster_sdk(self):
        ...
        try:
            ...
            # Switch to prepare mode first, then custom mode
            print("🔄 Switching robot to prepare mode first...")
            res = self.booster_client.ChangeMode(RobotMode.kPrepare)
            if res == 0:
                print("✅ Robot switched to prepare mode successfully!")
            else:
                print(f"⚠️  Prepare mode switch returned code: {res}")
            
            print("⏳ Waiting 2 seconds before switching to custom mode...")
            time.sleep(2.0)
